function myfunc11, input
  result1 = total(input)
  result2 = input[1]^input[0]
  return, [result1, result2]
end

function myfunc12, input, FUNCTARGS=fcnargs
...
output=myfunc11(input)
temp=size(output,/DIMENSIONS)
output_num=temp[0]
walk_num=30
iteration_num=500

mcmc_sim=mc->hammer('myfunc11', input, input_err_m, input_err_p, output, $
...
output=myfunc12(input, FUNCTARGS=fcnargs)
temp=size(output,/DIMENSIONS)
output_num=temp[0]
walk_num=30
iteration_num=100

mcmc_sim=mc->hammer('myfunc12', input, input_err_m, input_err_p, output, $
                    walk_num=walk_num, iteration_num=iteration_num, $
